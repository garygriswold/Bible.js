...
server.get('/version/:version', function(request, response, next) {
...
server.get('/locale/:locale', function(request, response, next) {
...
server.get('/draft/:draftId', function(request, response, next) {
	console.log('Get Draft ' + request.params.draftId);
	response.send(200, 'Get Draft ' + request.params.draftId);
	return(next());
});

server.del('/draft/:draftId', function(request, response, next) {
...
server.get(/^.*$/, function(request, response, next) {
	...
	response.send(404, 'No Valid Route for ' + request.url);
	return(next());
})
